...
import net.devrune.messenger_crow.network.ScrollLecternGUIButtonMessage;
...
public class ScrollLecternGUIScreen extends AbstractContainerScreen<ScrollLecternGUIMenu> {
	private final static HashMap<String, Object> guistate = ScrollLecternGUIMenu.guistate;
	...
	public ScrollLecternGUIScreen(ScrollLecternGUIMenu container, Inventory inventory, Component text, String color) {
	...
	private ResourceLocation texture() {
		if (this.color.isEmpty()) {
			return new ResourceLocation("messenger_crow:textures/screens/scroll_lectern_gui.png");
		} else {
			return new ResourceLocation("messenger_crow:textures/screens/" + this.color + "_scroll_lectern_gui.png");
		}
	}
	...
	public void init() {
		super.init();
		Username = new EditBox(this.font, this.leftPos + 59, this.topPos + 10, 88, 18, Component.translatable("gui.messenger_crow.scroll_lectern_gui.Username")) {
			@Override
			public void insertText(String text) {
				super.insertText(text);
				if (getValue().isEmpty())
					setSuggestion(Component.translatable("gui.messenger_crow.scroll_lectern_gui.Username").getString());
				else
					setSuggestion(null);
			}
		...
		Username.setSuggestion(Component.translatable("gui.messenger_crow.scroll_lectern_gui.Username").getString());
		Username.setMaxLength(16);
		Username.setBordered(false);
		Username.setTextColor(14270876);
		...
		button_empty = new PlainTextButton(this.leftPos + 5, this.topPos + 4, 25, 20, Component.translatable("gui.messenger_crow.scroll_lectern_gui.button_empty"), e -> {
			if (true) {
				...
				MessengerCrowMod.PACKET_HANDLER.sendToServer(new ScrollLecternGUIButtonMessage(0, x, y, z, textstate));
				ScrollLecternGUIButtonMessage.handleButtonAction(entity, 0, x, y, z, textstate);
			}
		}, this.font);
		guistate.put("button:button_empty", button_empty);
		this.addRenderableWidget(button_empty);
		button_empty1 = new PlainTextButton(this.leftPos + 148, this.topPos + 3, 25, 20, Component.translatable("gui.messenger_crow.scroll_lectern_gui.button_empty1"), e -> {
			if (true) {
				textstate.put("textin:Username", Username.getValue());
				textstate.put("textin:Line1", Line1.getValue());
				textstate.put("textin:Line2", Line2.getValue());
				textstate.put("textin:Line3", Line3.getValue());
				MessengerCrowMod.PACKET_HANDLER.sendToServer(new ScrollLecternGUIButtonMessage(1, x, y, z, textstate));
				ScrollLecternGUIButtonMessage.handleButtonAction(entity, 1, x, y, z, textstate);
			}
		}, this.font);
